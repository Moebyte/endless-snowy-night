...
(function () {
  ...
  Game.getAttendingWolves = function () {
    var g = ensureState();
    // [v9.6.1] All alive, non-exiled, non-bound wolves ATTEND the meeting.
    // Kill-heart no longer gates attendance — a hesitant wolf still shows up
    // to discuss, he just may not agree to the specific plan (see wolfResponse).
    // Previously this filtered by kill-heart, causing ~45% of nights to have
    // too few attendees -> safe night.
  ...
  Game.wolfResponse = function (wolfId, proposedTarget, leaderId) {
    ...
    // [v9.6.1] Base hesitation from kill-heart: low-heart wolves oppose more.
    // This makes early phases cautious (hard to reach majority) while late
    // phases sail through (high heart = rarely opposes).
    var _heart = Game.wolfKillHeart(wolfId);
    var _hesitate = 1 - _heart;  // 0.9 early, 0.05 late
    ...
    if (wolfId === "zhao_mingcheng") {
      if (proposedTarget === "chen_mo") {
        ...
        resp.vote = "approve";
      } else if (Math.random() < (_hesitate * 0.3) && leaderId !== "zhao_mingcheng") {
        resp.vote = "oppose";
      } else {
        resp.vote = "approve";
      }
      ...
      if (GOD_ROLES.indexOf(targetRole) !== -1) {
        resp.vote = "approve";
      } else if (Math.random() < (_hesitate * 0.5)) {
        resp.vote = "oppose";
      } else {
        resp.vote = "approve";
      }
  ...
  Game.resolveWolfDiscussion = function () {
    ...
    for (var round = 1; round <= maxRounds; round++) {
      ...
      // [v9.6.1] MAJORITY rules: approve > oppose is enough.
      // Wolves are a pack — the majority drags the hesitant one along.
      // (Previously required unanimous, causing ~45% safe nights.)
      if (approve > oppose) {
        return {
          leader: leader,
      ...
      // Majority not reached — try next round with a different leader
    }
