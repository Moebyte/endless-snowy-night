...
for (let i = 0; i < 20; i++) {
  ...
  for (let day = 2; day <= 7; day++) {
    ...
    log.push({ day, kill, swap, guardTarget, prophetCheck: prophetCheck ? prophetCheck.target : null, prophetShared, witch: witchAct });
...
// 汇总
let witchSaves = 0, witchCurses = 0, swaps = 0, guards = 0, deaths = 0, shares = 0, wk = 0, ff = 0, guarded = 0;
const killTargets = {}, saveT = {}, curseT = {};
results.forEach(r => r.log.forEach(l => {
  if (l.kill.killed) { deaths++; killTargets[l.kill.actualTarget] = (killTargets[l.kill.actualTarget]||0)+1; }
  if (l.swap) swaps++;
  if (l.guardTarget) guards++;
  if (l.prophetShared && l.prophetShared.ok) shares++;
  if (l.witch && l.witch.ok) {
    if (l.witch.action === 'save') { witchSaves++; saveT[l.witch.target] = (saveT[l.witch.target]||0)+1; }
    else { witchCurses++; curseT[l.witch.target] = (curseT[l.witch.target]||0)+1; }
  }
...
console.log('女巫救人: ' + witchSaves + ' | 女巫诅咒: ' + witchCurses + ' | 魔术师交换: ' + swaps + ' | 骑士守卫: ' + guards + ' | 预言家分享: ' + shares);
console.log('狼王同归: ' + wk + ' | 误杀同伴: ' + ff + ' | 骑士挡刀: ' + guarded);
console.log('\n女巫救人目标: ' + Object.entries(saveT).map(([k,v])=>(N[k]||{}).name+':'+v).join(', '));
console.log('女巫诅咒目标: ' + Object.entries(curseT).map(([k,v])=>(N[k]||{}).name+':'+v).join(', '));
...
results.forEach(r => {
  ...
  r.log.forEach(l => {
    ...
    if (l.prophetShared && l.prophetShared.ok) line += '(分享给' + (N[Game.prophetAICheckNight?.() || '']||{}).name + ')';
