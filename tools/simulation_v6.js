// ESN v6 - stronger day event influence, vote variety
const C={
...
const EV=[
  ...
  {t:"[RE]ZSfoundoldphotos",sus:"zs",amt:20},
  ...
  {t:"[RE]LXfoundscratchmessageGTOUT",trust:"lx",amt:15},
...
function wv(g,ev,day){
  ...
  tg.forEach(t=>{
    ...
    if(w==="zy"){if(t==="lx")s+=18;if(t==="cm")s+=8;}
    if(w==="gy"){s+=Math.random()*25;}
    if(w==="tx"){if(t==="cm")s-=75;if(t==="lx")s-=35;if(t==="sw")s+=30;if(t==="zs")s+=5;}
    if(w==="zm"){if(t==="zs")s+=30;if(t==="cm")s+=10;if(t==="jb")s+=8;}
    if(ev&&ev.sus&&t===ev.sus)s+=ev.amt||20;
    if(ev&&ev.trust&&t===ev.trust)s-=Math.min(s-5,(ev.amt||10)+5);
    if(day===2){if(t==="zs"||t==="cm")s+=12;if(t==="yz")s+=8;}
    if(day===3)s+=5; // generic tension
...
function genE(){const s=[...EV];for(let i=s.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[s[i],s[j]]=[s[j],s[i]];}
const r={};for(let d=2;d<=6;d++){const idx=Math.floor(Math.random()*s.length);r[d]=s.splice(idx,1)[0];}return r;}
...
for(let d=1;d<=7;d++){const dl=[];
  if(FD[d])dl.push(FD[d]);
  const re=ev[d];if(re)dl.push(re.t);
...
const S={d7:0,nw:0,ad:0,fh:0,yz:0,lx:0,ss:0,cm:0,sw:0,jb:0,zs:0,total:0,vt:0};
...
console.log("=== ESN v6 Stronger Events 20r ===");
console.log("DAY7:"+S.d7+" NW:"+S.nw+" AvgDth:"+S.avgd+"/12");
console.log("Deaths: FH:"+S.fh+" YZ:"+S.yz+" LX:"+S.lx+" SS:"+S.ss);
