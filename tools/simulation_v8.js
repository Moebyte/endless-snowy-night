// ESN v8 - wolves deduce roles from clues, may be wrong
const C={
...
const W=["zy","gy","tx","zm"];const ROLES=["pro","wit","kni","mag"];const GODS=["fh","yz","lx","ss"];
function p(a){return a[Math.floor(Math.random()*a.length)]}
function rm(a,i){return a.filter(x=>x!==i)}
function gs(){return {a:Object.keys(C),dd:[],wu:0,wr:0,kd:0,kl:0,kt:null,ms:[],pc:{},gs:null,zr:false,sus:{}};}
const EV=[
  {t:"[RE]TXwhispersSwhides",sus:"sw",amt:25,susClue:""},
  {t:"[RE]FHandZYargueloudly",sus:"zy",amt:20,susClue:"wk"},
  {t:"[RE]SScountingcards",sus:"ss",amt:20,susClue:"mag"},
  {t:"[RE]YZtreatingwound",trust:"yz",amt:15,susClue:"wit"},
  {t:"[RE]BrokenwindowZMseesJBrunning",sus:"jb",amt:25,susClue:""},
  {t:"[RE]LXfoundscratchmsgGTOUT",trust:"lx",amt:15,susClue:"kni"},
  {t:"[RE]GYaskingtoomanyQs",sus:"gy",amt:20,susClue:"mw"},
  {t:"[RE]SWcryinginchapel",trust:"sw",amt:15,susClue:""},
  {t:"[RE]TXluggageopened",sus:"tx",amt:20,susClue:"hw"},
  {t:"[RE]FHsenseevilbasement",sus:"zy",amt:20,susClue:"pro"},
  {t:"[RE]ZSoldphotos",sus:"zs",amt:20,susClue:""},
  {t:"[RE]CMfoundlockedroom",trust:"cm",amt:15,susClue:""}
];
function updateSuspicion(g,ev){
  if(!ev||!ev.susClue)return;
  if(Math.random()<0.4){
    const targets=ev.sus?[ev.sus]:ev.trust?[ev.trust]:[];
    targets.forEach(t=>{if(!g.sus[t])g.sus[t]={};g.sus[t][ev.susClue]=(g.sus[t][ev.susClue]||0)+1;});
  }
  if(Math.random()<0.2){
    const wrong=g.a.filter(i=>!W.includes(i)&&i!==(ev.sus||ev.trust));
    if(wrong.length){const t=p(wrong);if(!g.sus[t])g.sus[t]={};g.sus[t][ev.susClue]=(g.sus[t][ev.susClue]||0)+1;}
  }
}
function wv(g,ev,day){
  updateSuspicion(g,ev);
  ...
  tg.forEach(t=>{
    ...
    if(w==="tx"){if(t==="lx")s-=35;if(t==="sw")s+=30;}
    if(w==="zm"){if(t==="zs")s+=30;if(t==="jb")s+=8;}
    if(g.sus[t]){
      Object.entries(g.sus[t]).forEach(([role,count])=>{
        if(ROLES.includes(role))s+=count*15;
        if(["wk","mw","hw","w"].includes(role)&&!W.includes(t))s-=count*10;
      });
    }
    if(ev&&ev.sus&&t===ev.sus)s+=ev.amt||20;
    if(ev&&ev.trust&&t===ev.trust)s-=Math.min(s-5,(ev.amt||10)+5);
    if(day===2){if(t==="zs"||t==="yz")s+=12;}
    if(day===5){if(t==="jb"||t==="ss")s+=15;}
...
console.log("=== ESN v8 Clue deduction 20r ===");
console.log("DAY7:"+S.d7+" NW:"+S.nw+" AvgD:"+S.avgd+"/11");
console.log("");
console.log("Deaths:");
console.log(" FH:"+S.fh+" YZ:"+S.yz+" LX:"+S.lx+" SS:"+S.ss+" CM:"+S.cm);
