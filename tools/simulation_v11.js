// ESN v11 - Wolves frame people by killing those in conflict
const C={
...
function wv(g,ev,day){
  ...
  tg.forEach(t=>{let s=20+Math.random()*35;
    ...
    if(w==="tx"){if(t==="lx")s-=25;if(t==="sw")s+=15;if(t===ev?.p2)s+=10;}
    if(w==="zm"){if(t==="zs")s+=25;if(t==="jb")s+=8;}
    if(ev&&ev.type==="fight"){
      if(t===ev.p1||t===ev.p2)s+=25; // kill someone involved in the fight
    }
    if(ev&&ev.type==="help"){
      if(t===ev.p1)s+=20; // kill the helper
    }
    if(ev&&ev.type==="suspicious"){
      if(t===ev.p1)s+=20;
    }
...
console.log("=== ESN v11 FrameGame 20r ===");
