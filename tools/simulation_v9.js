// ESN v9 - wolves know nothing. Clues = vague suspicion only.
const C={
...
const EV=[
  ...
  {t:"[RE]LXfoundscratchmsg",trust:"lx",amt:15},
  {t:"[RE]GYaskingtoomanyQs",sus:"gy",amt:20},
  {t:"[RE]SWcryinginchapel",trust:"sw",amt:15},
  {t:"[RE]TXluggageopened",sus:"tx",amt:20},
  {t:"[RE]FHsenseevil",sus:"zy",amt:20},
  {t:"[RE]ZSoldphotos",sus:"zs",amt:20},
  {t:"[RE]CMfoundlockedroom",trust:"cm",amt:15}
];
function addSus(g,ev){if(!ev)return;
  if(ev.sus&&g.a.includes(ev.sus)){g.sus[ev.sus]=(g.sus[ev.sus]||0)+1;}
  if(ev.trust&&g.a.includes(ev.trust)){g.sus[ev.trust]=(g.sus[ev.trust]||0)-0.5;}
}
function wv(g,ev,day){
  ...
  tg.forEach(t=>{let s=20+Math.random()*35;
    ...
    if(w==="tx"){if(t==="lx")s-=30;if(t==="sw")s+=25;}
    if(w==="zm"){if(t==="zs")s+=25;if(t==="jb")s+=8;}
    if(g.sus[t])s+=g.sus[t]*12;
    if(ev&&ev.sus&&t===ev.sus)s+=ev.amt;
    if(ev&&ev.trust&&t===ev.trust)s-=Math.min(s-5,20);
    if(day===2)s+=Math.random()*10;
    if(day===5&&t==="jb")s+=10;
...
const nl=["N"+d];const w=wv(g,ev[d],d);if(!w){lg.push(" wdead/notarget");lg.push("");continue;}
...
console.log("=== ESN v9 NoRoleKnowledge 20r ===");
console.log("Status: DAY7:"+S.d7+" NW:"+S.nw+" AvgDth:"+S.avgd+"/11  CM:"+S.cm+"/20");
console.log("");
