// ESN v3 - balanced narrative tension
const C={
...
tg.forEach(t=>{let s=25+Math.random()*25;
// Personal goals cause vote splitting, but everyone wants to kill
if(w==="zy"){if(t==="lx")s+=25;} // WK wants strong targets
if(w==="gy"){s+=Math.random()*20;} // MW random aggressive
if(w==="tx"){if(t==="cm")s-=70;if(t==="lx")s-=40;if(t==="sw")s+=45;} // TX: crush/jealousy
if(w==="zm"){if(t==="zs")s+=30;if(t==="cm"||t==="sw")s+=10;} // ZM: easy prey
...
const top=Object.entries(tl).filter(([_,x])=>x===mv).map(([k])=>k);
return{target:p(top),tl,detail:vo};}
...
const S={d7:0,nw:0,to:0,ad:0,fh:0,yz:0,lx:0,ss:0,cm:0,sw:0,jb:0,zs:0,gd:0,kd:0,md:0,hd:0};
...
r.l.forEach(l=>{if(l.startsWith(" [X]")){}if(l.startsWith(" [W]YZs"))S.gd++;if(l.startsWith(" [W]YZc"))S.kd++;if(l.startsWith(" [MW]"))S.md++;if(l.startsWith(" [HJ]"))S.hd++;})});
S.ad=(S.ad/20).toFixed(1);
console.log("=== ESN v3 平衡版 20轮 ===");
console.log("DAY7:"+S.d7+" NEAR_WIPE:"+S.nw+" TO:"+S.to);
console.log("平均:"+S.ad+"d");
console.log("");
console.log("死亡:");
console.log(" FH:"+S.fh+" YZ:"+S.yz+" LX:"+S.lx+" SS:"+S.ss);
console.log(" CM:"+S.cm+" SW:"+S.sw+" JB:"+S.jb+" ZS:"+S.zs);
console.log("");
console.log("女巫救:"+S.gd+" 诅咒:"+S.kd+" 机械狼:"+S.md+" 悍跳:"+S.hd);
console.log("");
for(let i=0;i<20;i++){const r=R[i];
console.log("R"+(i+1)+"|"+r.o+"|d"+r.ds);
console.log("A:["+r.a.map(id=>C[id].n).join()+"]");
console.log("D:["+r.dd.map(id=>C[id].n).join()+"]");
