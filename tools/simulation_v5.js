// ESN v5 - dynamic day events affect wolf voting
const C={
...
function gs(){return {a:Object.keys(C),dd:[],wu:0,wr:0,kd:0,kl:0,kt:null,ms:[],pc:{},gs:null,zr:false,sv:{},events:[]};}
const FIXED_EVENTS={
  2:"[FD] Body found! Panic spreads",
  3:"[FD] Room search proposed, tensions rise",
  4:"[FD] Supplies running low, rationing begins",
  5:"[FD] Someone tried to escape the manor",
  6:"[FD] Survivors split into factions"
};
const RANDOM_EVENTS=[
  {t:"[RE] TX pulls you aside, whispers SW is hiding something",sus:"sw",trust:"tx"},
  {t:"[RE] ZS found old photographs in the cellar",sus:"zs",trust:""},
  {t:"[RE] Loud argument heard between FH and ZY",sus:"zy",trust:"fh"},
  {t:"[RE] You notice SS counting cards in his room",sus:"ss",trust:""},
  {t:"[RE] YZ was seen treating someone's wound late at night",trust:"yz"},
  {t:"[RE] Someone broke a window - ZM claims he saw JB running",sus:"jb",trust:"zm"},
  {t:"[RE] LX found a scratched message on the wall: get out",sus:"",trust:"lx"},
  {t:"[RE] CM discovered a locked room in the east wing",sus:"",trust:"cm"},
  {t:"[RE] GY is asking too many questions about everyone's background",sus:"gy"},
  {t:"[RE] SW was crying in the chapel this morning",sus:"",trust:"sw"},
  {t:"[RE] TX's luggage was found opened, contents spilled",sus:"tx"},
  {t:"[RE] FH claims he sensed something evil near the basement",sus:"zy",trust:"fh"}
];
function genEvents(){
  const used=[];const pool=[...RANDOM_EVENTS];
  const shuffle=a=>{for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}return a;};
  shuffle(pool);
  // Each day (2-6) gets one random event
  for(let d=2;d<=6;d++){
    if(pool.length>0)used[d]=pool.shift();
    else used[d]=null;
  }return used;}
function wv(g,re,day){
  ...
  tg.forEach(t=>{let s=25+Math.random()*30;
    // Base personal goals
    if(w==="zy"){if(t==="lx")s+=15;}
    if(w==="gy"){s+=Math.random()*20;}
    if(w==="tx"){if(t==="cm")s-=80;if(t==="lx")s-=40;if(t==="sw")s+=35;}
    if(w==="zm"){if(t==="zs")s+=30;if(t==="cm"||t==="sw")s+=10;}
    if(re&&re.sus&&t===re.sus)s+=15;
    if(re&&re.trust&&t===re.trust)s-=10;
    if(day===2&&(t==="zs"||t==="cm"))s+=10; // body found near them
    if(day===3&&t==="sw")s+=8; // room search suspicion
    if(day===5&&(t==="jb"||t==="ss"))s+=12; // escape attempt
...
function sim(sd){let s=sd;function rn(){s=(s*16807)%2147483647;return(s-1)/2147483646;}const g=gs();const ev=genEvents();const lg=[];
for(let d=1;d<=7;d++){const dl=[];
  if(d>=2&&d<=6)dl.push(FIXED_EVENTS[d]||"");
  const re=ev[d];
  if(re)dl.push(re.t);
...
const nl=["N"+d];const w=wv(g,re,d);if(!w){lg.push(" wdead");lg.push("");break;}
...
console.log("=== ESN v5 Dynamic Day Events 20r ===");
console.log("DAY7:"+S.d7+" NW:"+S.nw+" AvgD:"+S.avgd+" AvgDay:"+S.ad);
console.log("");
console.log("Deaths:");
...
console.log("==R"+(i+1)+" "+r.o+" d"+r.ds);
console.log("A:"+r.a.length+"["+r.a.map(id=>C[id].n).join()+"] D:"+r.dd.length+"["+r.dd.map(id=>C[id].n).join()+"]");
