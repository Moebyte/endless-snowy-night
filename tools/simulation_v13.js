// ESN v13 - fake jump: wolves kill the ACCUSED target, not the real prophet
const C={
...
function fakeJump(g,d){
  if(d<2||!g.fj){g.fj=null;return null;}
  return g.fj;
}
function genFJ(g,d){
  if(d<2||Math.random()>0.4){g.fj=null;return null;}
  const candidates=[];
  if(g.a.includes("zm"))candidates.push({id:"zm",name:"Zhao"});
  if(g.a.includes("gy"))candidates.push({id:"gy",name:"Gu"});
  if(!candidates.length){g.fj=null;return null;}
  const fj=p(candidates);
  const targets=g.a.filter(i=>!W.includes(i)&&i!==fj.id&&i!=="cm");
  if(!targets.length){g.fj=null;return null;}
  const target=p(targets);
  g.fj={jumper:fj.id,name:fj.name,target};
  return g.fj;
}
...
const fjDay=2+Math.floor(Math.random()*4);
g.fj=null;
for(let d=1;d<=7;d++){const dl=[];
  ...
  if(d===fjDay){
    const fj=genFJ(g,d);
    if(fj){
      dl.push("[HJ]"+fj.name+" claims prophet, accuses "+C[fj.target].n+"!");
    }
  }
...
if(d>=7)return{o:ah>0?"DAY7":"WIPE",ds:7,dd:g.dd,a:g.a,l:lg,fj:g.fj};
...
const S={d7:0,nw:0,ad:0,fh:0,yz:0,lx:0,ss:0,cm:0,sw:0,jb:0,zs:0,total:0,fj_zm:0,fj_gy:0,fj_no:0};
...
console.log("=== ESN v13 KillAccused 20r ===");
console.log("DAY7:"+S.d7+" AvgD:"+S.avgd+"/11  CM:"+S.cm);
...
console.log("==R"+(i+1)+" "+r.o+" d"+r.ds+" A"+r.a.length+" D"+r.dd.length+" FJ:"+(r.fj?r.fj.name:"none"));
