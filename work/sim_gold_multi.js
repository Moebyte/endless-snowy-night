...
var deepHooks = 0;
var goldWaterShared = 0;
var curseSuppressed = 0;

for (var run = 0; run < 10; run++) {
  S.variables.game = GS.create();
  var g = S.variables.game;
  var hookThisRun = false;
  ...
  for (var day = 1; day <= 7; day++) {
    ...
    if (G.isAlive('fang_heng')) {
      ...
      if (ct) {
        ...
        else { var gs = G.prophetAIShareAllyInfo(); if (gs) goldWaterShared += gs.length; }
      }
    }
    
    if (G.isAlive('tang_xiaotang') && G.isGoldWater('tang_xiaotang')) {
      var m = G.hiddenWolfAIMislead();
      if (m) { deepHooks++; hookThisRun = true; }
    }
    ...
    if (G.isAlive('ye_zhiqiu') && !G.witchBroken()) {
      ...
      // Check curse target before and after
  ...
  console.log('Run ' + (run+1) + ': 好人' + aliveGood(g).length + ' 狼人' + aliveWolves(g).length + 
    (hookThisRun ? ' [隐狼倒钩!]' : '') + ' | 隐狼' + (G.isAlive('tang_xiaotang')?'存活':'已死'));
}

console.log('\n=== 统计（10轮）===');
console.log('金水信息分享次数: ' + goldWaterShared);
console.log('隐狼深水倒钩次数: ' + deepHooks);
